...
@if ($cart)
    @if (request()->is('checkout/redirect/stripe'))
        <html>
            ...
            <script>
                @if ( company()->getSuperConfigData('sales.paymentmethods.stripe.mode') == 0 )
                    var publish_key = '{{ company()->getSuperConfigData('sales.paymentmethods.stripe.test_publishable_key') }}';
                    var stripe = Stripe(publish_key);
                @else
                    var publish_key = '{{ company()->getSuperConfigData('sales.paymentmethods.stripe.live_publishable_key') }}';
