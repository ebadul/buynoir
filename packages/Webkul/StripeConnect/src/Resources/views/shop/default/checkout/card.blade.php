    ...
    @if(request()->is('checkout/redirect/stripe'))
        <html>
            ...
            <script>
                    ...
                    @if($stripeSuperAdmin->first()->sandbox == '0')
                        var stripe = Stripe('$stripeSuperAdmin->first()->test_publishable_key');
                    @else
                        var stripe = Stripe('$stripeSuperAdmin->first()->live_publishable_key');
                    @endif
