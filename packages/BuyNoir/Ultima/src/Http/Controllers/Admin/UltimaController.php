...
namespace BuyNoir\Ultima\Http\Controllers\Admin;
...
class UltimaController extends Controller
{
    ...
    public $ultimaMetadataRepository;
    ...
    {
        $ultimaAdminHelper = app('BuyNoir\Ultima\Helpers\UltimaAdminHelper');
        
        $ultimaMetaData = $ultimaAdminHelper->getUltimaMetaData($this->locale, $this->channel, false);

        if (! $ultimaMetaData) {
            $this->createMetaData($this->locale, $this->channel);

            $ultimaMetaData = $ultimaAdminHelper->getUltimaMetaData($this->locale, $this->channel);
        }

        $ultimaMetaData->advertisement = $this->manageAddImages(json_decode($ultimaMetaData->advertisement, true) ?: []);

        return view($this->_config['view'], [
            'metaData' => $ultimaMetaData,
        ]);
    }
    ...
    public function storeMetaData($id)
    {
        ...
        $ultimaMetaData = $ultimaMetadataRepository->findOneWhere([
            'id' => $id,
        ]);
        ...
        // update row
        $product = $ultimaMetadataRepository->update($params, $id);
